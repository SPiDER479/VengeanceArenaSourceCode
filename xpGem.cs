...
{
    [SerializeField] private AudioSource xpGainedAudio;
    ...
    void Update()
    {
        if (!Manager.pause)
        {
            ...
            if (Math.Abs(Math.Abs(Manager.playerY) - Math.Abs(transform.position.y)) <= 0.5f &&
                Math.Abs(Math.Abs(Manager.playerX) - Math.Abs(transform.position.x)) <= 0.5f && !xpGainedAudio.isPlaying)
            {
                xpGainedAudio.Play();
                Manager.playerXP += 10 + Manager.playerExtraXP/10;
            }
            if (xpGainedAudio.time >= 0.7)
